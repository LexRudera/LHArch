...
Object* Button::doCopy(Object* o) {
	Button* b = new Button(m_Parent, getString(), getSize(), getPosition(), getRotation());
	b->setOnClickFunction(m_OnClick);
	b->m_DownStyle = m_DownStyle;
	b->m_IdleStyle = m_IdleStyle;
	b->m_HoverStyle = m_HoverStyle;
	ControlBase::doCopy(b);
	return b;
}
	...
	if (MPos.x > getPosition().x // Below the x pos
		...
		if (Core::get()->getInputManager()->isButtonUp(sf::Mouse::Button::Left)) {
			if (m_Parent->buttonReady()) { // If a button already have been pressed this frame, don't even try anything
				if (m_OnClick != 0) { // If there is an assigned function or not.
					(m_Parent->*m_OnClick)();
				}
			}
		}
